...
const TAGS = ['Hotels', 'Resorts', 'Guest Facilities', 'Public Spaces'];
const TAGS_AR = 'الفنادق • المنتجعات • مرافق الضيوف • المساحات العامة';

const DELIVERABLES = [
  { num: '01', title: 'Guest-Ready Presentation', body: 'Lobbies, guest rooms and public spaces maintained to a standard your guests notice.', bodyAr: 'الردهات والغرف والمساحات العامة بمستوى يلاحظه ضيوفك.' },
  { num: '02', title: 'Coordinated Front & Back of House', body: 'One team covering housekeeping, public areas and back-of-house operations together.', bodyAr: 'فريق واحد يغطي التدبير المنزلي والمساحات العامة وعمليات الخلفية معًا.' },
  { num: '03', title: 'Flexible, Round-The-Clock Schedules', body: 'Service that moves with occupancy, events and peak hours.', bodyAr: 'خدمة تتكيف مع الإشغال والفعاليات وساعات الذروة.' },
];
...
function SectorHospitalityDesktop() {
  return (
    <div style={{ fontFamily: "'Poppins', sans-serif", color: '#111412', overflowX: 'auto', minWidth: 1360 }}>
      ...
      <div style={{ position: 'relative', background: '#111412', overflow: 'hidden', padding: '60px 64px 100px', clipPath: 'polygon(0 0, 100% 0, 100% 94%, 0 100%)' }}>
        <img src="/assets/sectors-hospitality.jpg" alt="Hotel lobby" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0.55 }} />
        ...
        <div style={{ position: 'relative', maxWidth: 760, animation: 'std-rise 0.8s ease-out both' }}>
          <div style={{ fontSize: 12.5, color: '#C9C9C9', letterSpacing: 1, marginBottom: 22 }}><Link to={ROUTES.home} style={{ color: '#C9C9C9' }}>Home</Link> / <Link to={ROUTES.sectors} style={{ color: '#C9C9C9' }}>Sectors</Link> / Hotels &amp; Hospitality</div>
          ...
          <h1 style={{ fontFamily: "'Archivo',sans-serif", fontSize: 46, fontWeight: 800, color: '#fff', lineHeight: 1.1, margin: '0 0 14px', textTransform: 'uppercase' }}>Hotels &amp; Hospitality</h1>
          <div style={{ fontFamily: "'Noto Kufi Arabic',sans-serif", direction: 'rtl', fontSize: 24, fontWeight: 600, color: '#8BC53F', marginBottom: 24 }}>الفنادق والضيافة</div>
      ...
      <div style={{ background: '#FFFFFF', padding: '90px 64px' }}>
        <div style={{ maxWidth: 1240, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 70, alignItems: 'center' }}>
          <div style={{ animation: 'std-rise 0.8s ease-out both' }}>
            <img src="/assets/sectors-hospitality.jpg" alt="" style={{ width: '100%', height: 400, objectFit: 'cover', display: 'block' }} />
            <div style={{ marginTop: -56, marginLeft: 24, position: 'relative', background: '#111412', color: '#fff', display: 'inline-flex', flexDirection: 'column', padding: '20px 26px' }}>
              ...
              <span style={{ fontSize: 11.5, letterSpacing: 1, color: '#C9C9C9' }}>FLEXIBLE SERVICE COVERAGE</span>
            </div>
          </div>
      ...
      <div style={{ background: '#2A2E2A', padding: '90px 64px', textAlign: 'center' }}>
        <h2 style={{ fontFamily: "'Poppins',sans-serif", fontSize: 28, fontWeight: 700, color: '#fff', letterSpacing: 0.5, textTransform: 'uppercase', margin: '0 0 26px' }}>Ready to bring one accountable team to your property?</h2>
        <CtaButton />
      </div>
...
function SectorHospitalityMobile() {
  return (
    <div style={{ width: '100%', maxWidth: 560, margin: '0 auto', background: '#FFFFFF', fontFamily: "'Poppins',sans-serif", color: '#111412', overflowX: 'hidden', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', background: '#111412', overflow: 'hidden', padding: '44px 20px 52px', clipPath: 'polygon(0 0, 100% 0, 100% 97%, 0 100%)' }}>
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ fontSize: 11.5, color: '#C9C9C9', letterSpacing: 0.5, marginBottom: 16 }}><Link to={ROUTES.home} style={{ color: '#C9C9C9' }}>Home</Link> / <Link to={ROUTES.sectors} style={{ color: '#C9C9C9' }}>Sectors</Link> / Hospitality</div>
          <div style={{ fontSize: 11.5, letterSpacing: 3, fontWeight: 600, color: '#8BC53F', marginBottom: 12 }}>SECTORS WE SERVE</div>
          <h1 style={{ fontFamily: "'Archivo',sans-serif", fontSize: 28, fontWeight: 800, color: '#fff', lineHeight: 1.15, margin: '0 0 12px', textTransform: 'uppercase' }}>Hotels &amp; Hospitality</h1>
          <div style={{ fontFamily: "'Noto Kufi Arabic',sans-serif", direction: 'rtl', fontSize: 18, fontWeight: 600, color: '#8BC53F', marginBottom: 18 }}>الفنادق والضيافة</div>
      ...
      <div style={{ background: '#FFFFFF', padding: '44px 20px' }}>
        <img src="/assets/sectors-hospitality.jpg" alt="" style={{ width: '100%', height: 200, objectFit: 'cover', display: 'block', marginBottom: 24 }} />
      ...
      <div style={{ background: '#2A2E2A', padding: '52px 20px', textAlign: 'center' }}>
        <h2 style={{ fontFamily: "'Poppins',sans-serif", fontSize: 20, fontWeight: 700, color: '#fff', letterSpacing: 0.3, textTransform: 'uppercase', margin: '0 0 22px', lineHeight: 1.35 }}>Ready to bring one accountable team to your property?</h2>
        <CtaButton mobile />
      </div>
...
export default function SectorHospitality() {
  return (
    <>
      <div className="desktop-only"><SectorHospitalityDesktop /></div>
      <div className="mobile-only"><SectorHospitalityMobile /></div>
    </>
  );
}
